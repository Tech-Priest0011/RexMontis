...
public class Player1_gravityController : MonoBehaviour
{
    // ===================================================================== **
    ...
    void FixedUpdate(){
        ...
        if (gravityZone.GetComponent<Player2_gravity>().isAttracting) {
        ...
        if (gravityZone.GetComponent<Player2_gravity>().isPushing) {
            rb.AddForce(gravityZone.GetComponent<Player2_gravity>().direction * gravityZone.GetComponent<Player2_gravity>().strength);
        }

        // Se fait lancer/tirer par l'ennemi
        if (inThrowZone && Input.GetKey(KeyCode.V)) {
            rb.AddForce(gravityZone.GetComponent<Player2_gravity>().direction * gravityZone.GetComponent<Player2_gravity>().strength * throwingStrength);
        }
